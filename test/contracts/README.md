...
| `bootstrap.sh` | all four `*-on-event.sh` stage the download in a temp and rename · a failure never ends a hook non-zero, but is reported | curl, sha256sum/shasum; network for the concurrency contract | yes |
