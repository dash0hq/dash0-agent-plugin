...
description: Configure the Dash0 → Cursor telemetry integration by writing the OTLP URL and auth token to ~/.cursor/dash0-agent-plugin.local.md. Use when the user wants to set up Dash0, enable telemetry, paste credentials, fix an inactive plugin install, or act on a "dash0: no team configured" message — spans carry no dash0.team.name until the team name is set.
---
...
2. Ask the user for these values one at a time. Do not assume any defaults beyond the ones listed.
   ...
   - **Dataset** (recommended) — leave it blank and the backend picks its default; there is no literal `default` value to write
   - **Team name** (recommended) — tags every span with `dash0.team.name`; without it spans carry no team attribution
