...
function Install-PluginFile {
  ...
  & curl.exe -fsS -L -o $Destination "$RawBase/$Source"
  if ($LASTEXITCODE -ne 0) { Stop-WithError "failed to download: $RawBase/$Source" }
...
$manifest = Get-Content -LiteralPath $HooksManifestPath -Raw | ConvertFrom-Json
...
if ($Endpoint -and $Token) {
  ...
  $env:CURSOR_PLUGIN_OPTION_AUTH_TOKEN = $Token
  $env:DASH0_DATASET = $Dataset
  $env:DASH0_PLUGIN_DATA = $scratch
  try {
    $checkOutput = ($event | & $BinPath 2>&1 | Out-String)
  } finally {
    Remove-Item Env:\DASH0_OTLP_URL, Env:\CURSOR_PLUGIN_OPTION_AUTH_TOKEN, `
