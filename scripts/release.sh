...
sed -i '' "s/VERSION=\"[^\"]*\"/VERSION=\"${VERSION}\"/" install-cursor.sh
...
grep 'VERSION=' install-cursor.sh

git add .claude-plugin/plugin.json .cursor-plugin/plugin.json scripts/on-event.sh scripts/cursor-on-event.sh install-cursor.sh
