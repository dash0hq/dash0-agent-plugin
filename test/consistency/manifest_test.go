...
// The bootstrap downloads the binary for its own VERSION=, so a bumped manifest
// over a stale bootstrap ships old code under a new version number.
...
// Both marketplaces resolve a plugin by this name, so a rename in one manifest
// breaks that runtime's install with a bare "plugin not found".
...
// One version across the four manifests: they are released together from one tag,
// so a per-runtime version makes the release assets ambiguous.
...
// The manifest points at the hooks file the runtime reads. Cursor is the
// exception: it silently ignores `hooks` in a local-plugin manifest, so the field
// stays absent and install-cursor.sh reads the file directly. Either way the file
// has to exist, being the source of truth for which events get instrumented.
...
// Claude and Cursor auto-discover a skills/ directory at the plugin root when the
// manifest does not override it, which ships the other runtime's assets, so the
...
// The same auto-discovery trap as skills. Cursor declares an empty list purely to
// block it, so "declared" is the assertion and the list may be empty.
...
// userConfig is a Claude Code field. Declaring it elsewhere reads as a supported
// config path that the runtime never populates; those runtimes deliver credentials
