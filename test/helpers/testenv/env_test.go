...
// Which variables Clean removes. Every test that runs plugin code in a child
// process depends on it, so a name slipping through is a suite reading
// configuration out of the developer's shell.
func TestCleanEnv(t *testing.T) {
	// One per family, plus the prefixed option form for each agent, that being the
	// form harness.PluginOption prefers.
	dropped := []string{
		...
		"CLAUDE_PLUGIN_DATA",
		"COPILOT_PLUGIN_DATA",
		// Session markers an enclosing Claude Code exports. Leaked, they make a
		// nested Claude Code disable transcript saving, which costs the plugin the
		// turn content it reports on.
		...
		// Matches no prefix above (CLAUDE_CONFIG_, not CLAUDE_CODE_) and outranks
		// the temp HOME, so the install, the onboarding state and the credential
		// file would come from the developer's own ~/.claude.
		"CLAUDE_CONFIG_DIR",
	}
	// Needed by a child process, or naming a home rather than plugin configuration.
...
// A caller can still set a value Clean strips: os/exec keeps the last occurrence
// of a duplicate key.
