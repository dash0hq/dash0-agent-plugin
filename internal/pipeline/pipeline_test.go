...
func TestProcess_SessionStart_SubsequentFireIsNoOp(t *testing.T) {
	...
	// First fire: initializes normally.
	...
	// Second fire (simulates resume/compact/clear): should produce no messages
	// and must NOT overwrite the trace context.
...
// 12. A re-fired SessionStart still logs the event to filelog — the guard only
//     suppresses initialization, not event recording.
...
// 13. After SessionEnd cleans up sessionDir, a new SessionStart for a fresh
//     session with the same ID re-initializes normally (sentinel was removed).
func TestProcess_SessionStart_ReInitializesAfterSessionEnd(t *testing.T) {
	...
	// New session start after end: should initialize again.
	res := s.feed(t, map[string]any{"hook_event_name": "SessionStart", "session_id": "sess-1", "model": "sonnet"})
	require.Len(t, res.Messages, 1)
	assert.Equal(t, "dash0: connected", res.Messages[0].UserText)
