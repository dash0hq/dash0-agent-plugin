...
// The single plugin entry a marketplace manifest carries, plus the source path it
// resolves to.
...
// The two self-hosted marketplaces. An install resolves the entry's name and
// follows its source to a directory carrying a plugin manifest; a mismatched name
// or a dangling source surfaces only as an install failure on a user's machine.
func TestMarketplaceResolvesThePlugin(t *testing.T) {
	for _, a := range agentsWith(t, 2, func(a Agent) bool { return a.Marketplace != "" }) {
		t.Run(a.Label, func(t *testing.T) {
			...
			// Both CLIs resolve the source against the checkout root, not against the
			...
			// And the manifest has to sit where the runtime looks for it inside that
			// directory.
...
// The marketplace that repeats the plugin version, where drift serves a stale
// version to every installer.
