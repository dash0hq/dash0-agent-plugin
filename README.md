...
- **Amp / ampcode CLI and Orbs** — native plugin, source installation, and opt-in per-model usage in [`amp/README.md`](./amp/README.md). Completed-turn telemetry; export-based usage is partial and uses an unstable Amp export schema.
...
|---|---|---|
| `amp/` (`index.ts`, built `amp-on-event` helper) | Amp CLI and Orbs | Native plugin events, bounded completed-turn batches, optional exact-message per-model export usage |
