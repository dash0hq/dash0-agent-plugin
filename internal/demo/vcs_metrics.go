...
// EmitVCSMetrics generates synthetic dash0.gen_ai.vcs.* metrics for a WorkUnit,
...
func EmitVCSMetrics(wu WorkUnit, cfg otlp.Config, ts time.Time) error {
	payload, err := buildVCSMetricsPayload(wu, ts)
	if err != nil {
...
func buildVCSMetricsPayload(wu WorkUnit, ts time.Time) ([]byte, error) {
	...
	dims := []attribute{
		{Key: "dash0.gen_ai.vcs.repository.url.full", Value: strVal(wu.Repo.URL())},
		{Key: "dash0.gen_ai.vcs.repository.name", Value: strVal(wu.Repo.Name)},
		{Key: "dash0.gen_ai.vcs.owner.name", Value: strVal(wu.Repo.Owner)},
		{Key: "dash0.gen_ai.vcs.provider.name", Value: strVal(wu.Repo.Provider)},
		{Key: "dash0.gen_ai.vcs.ref.head.name", Value: strVal(wu.Branch)},
	}

	var metrics []metric

	// 1. dash0.gen_ai.vcs.change.count — PR merged
	...
	metrics = append(metrics, metric{
		...
		Histogram: &histogramMetric{
			DataPoints: []histogramDataPoint{{
				...
				TimeUnixNano:      tsNano,
				Count:             "1",
				Sum:               fmt.Sprintf("%d", mergeSeconds),
	...
	metrics = append(metrics, metric{
		...
		Histogram: &histogramMetric{
			DataPoints: []histogramDataPoint{{
				...
				TimeUnixNano:      tsNano,
				Count:             "1",
				Sum:               fmt.Sprintf("%d", approvalSeconds),
